...
const MARKER = 'ccforever';

/** フックコマンド。PATH 解決を優先し、焼き込みパスはフォールバック(cc-fleet の教訓) */
function hookCommand(nodePath, binPath) {
  return `command -v ccforever >/dev/null 2>&1 && ccforever backup --quiet || "${nodePath}" "${binPath}" backup --quiet || true`;
}
...
/** settings に ccforever の SessionStart フック(async)を加えた新オブジェクトを返す(冪等・非破壊) */
function withHook(settings, command) {
  ...
  next.hooks.SessionStart.push({
    matcher: '',
    hooks: [{ type: 'command', command, async: true, statusMessage: 'ccforever: archiving transcripts…' }],
  });
  return next;
}

/** ccforever のエントリを取り除いた新オブジェクトを返す */
